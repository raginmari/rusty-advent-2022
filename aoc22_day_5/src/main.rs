...
fn main() {
    ...
    println!("The result is {}", solve_star_1(ship, input));
}

#[derive(Debug)]
struct Ship
{
    container_stacks: Vec<ContainerStack>
}
...
fn solve_star_1<I>(mut ship: Ship, step_strings: I) -> String where I: IntoIterator<Item = String>
{
    ...
    for step in steps {
        for _ in 0..step.count {
            let to_move = ship.container_stacks[step.from_stack].containers.pop().unwrap();
            ship.container_stacks[step.to_stack].containers.push(to_move)
        }
    }
...
fn create_ship(stacks: &Vec<&str>) -> Ship
{
    ...
    for stack in stacks {
        ...
        let container_stack = ContainerStack { containers };
